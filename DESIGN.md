...
- **Active:** full-bleed background photograph with a gradient running inward from the menu side. Four recordings, vertically centred, left or right per the background's filename — each one a play/stop glyph on a translucent disc and nothing else. The titles that used to sit beside them are gone: four identical pressable shapes read as a set of things to try *together*, which is what a mix asks for, where a list of named rows reads as a queue to work through. Playing disc tints `accent`; pressing lightens rather than darkens, so feedback survives on a dark backdrop.
...
Audited at Stage 7 and again at Stage 9 — every interactive element carries `min-height: var(--touch-target-min)`: admin buttons and *Volver* (64px), keypad keys (76px), station icons (88px), carousel arrow pills (64px tall, wider than that with their labels).
